...
import {
  ...
  import { app } from "../firebase"; // Assuming you have this import
  ...
  export default function UpdateListing() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          if(+formData.regularPrice < +formData.discountPrice) return setError('Discount price must be lower then regualr price')  
          setLoading(true);
          setError(false);
          const res = await fetch(`/api/listing/update/${params.listingId}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json', // Remove the extra space
            },
            body: JSON.stringify({
              ...formData,
              ...
              <div className="flex flex-wrap gap-6">
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    id="bedrooms"
                    className="p-3 border border-gray-300 rounded-lg w-16 h-10"
                    min="1"
                    max="10"
                    required
                    onChange={handleChange}
